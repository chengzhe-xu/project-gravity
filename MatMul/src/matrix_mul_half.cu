...
__global__ void matrix_mul_half_kernel_32x32(__half* matAT, __half* matB, __half* matC, int M, int N, int K) {
...
matrix_template matrix_mul_half_host(const matrix_template& matAT, const matrix_template& matB, matrix_template& matC, int M, int N, int K) {
    ...
    start_timer(&timer);
    matrix_mul_half_kernel_32x32<<<grid_size, block_size>>>(device_matAT_h, device_matB_h, device_matC_h, M, N, K);
    ...
    printf("cuda kernel <matrix_mul_half_kernel_32x32> runtime %f ms.\n", kernel_time_ms);
    return matC;
}
