...
matrix_template matrix_mul_naive_host(const matrix_template& matAT, const matrix_template& matB, matrix_template& matC, int M, int N, int K) {
    ...
    __half* device_matAT = 0;
    __half* device_matB = 0;
    __half* device_matC = 0;
    cudaMalloc((void**)&device_matAT, M * K * sizeof(__half));
    cudaMalloc((void**)&device_matB, N * K * sizeof(__half));
    ...
    // cuda mem copy
    cudaMemcpy(device_matAT, matAT.data(), M * K * sizeof(__half), cudaMemcpyHostToDevice);
    cudaMemcpy(device_matB, matB.data(), N * K * sizeof(__half), cudaMemcpyHostToDevice);
    // kernel call
